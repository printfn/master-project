...
\definecolor{tableheadercolour}{rgb}{0.9569,0.9569,0.9569}
...
A REPL system is a very good tool for beginners to learn a new programming language. Without needing to install 42 locally, users can try out simple programs via their web browser. It works on all devices, including tablets and phones. Syntax highlighting is available, making it easier to write syntactically correct code. A wealth of sample programs is available to give users a head start.

The solutions presented in this report combine a REPL system with a bug bounty server.
...
When 42 detects that a contraint no longer holds, a constraint error is thrown. This is similar to an exception in Java, and can be caught with a \code{catch} expression. Note that inside of the \code{catch} block we no longer have access to the invalid instance.
...
The user interacts with the 42 bug bounty programme via a web page. It can be reached from the 42 website, \code{https://forty2.is}.
...
The web page is built with HTML, CSS and JavaScript. It uses asynchronous AJAX requests to communicate with the server. The editing UI is built around the Ace editor, and has support for syntax highlighting and themes. I have tested it in all modern web browsers (i.e. Chrome, Firefox and Safari), as well as on phones and tablets.

The front-end user interface is implemented as a static website. The code that users enter is sent in JSON format to the back-end server. We are using HTTPS to encrypt all network traffic. Upon receiving the client's JSON message, the server begins by decoding it. It first parses the received JSON message into a Java object, returning an error if the message is invalid. The server then evaluates the given 42 code against malicious code injection (see chapter \ref{C:injection} for details).

When no code injection is detected, the server starts executing the code. To sandbox it from the server process, a separate JVM is started just for this request. The two virtual machines communicate via the Java Remote Method Invocation (RMI) mechanism \cite{java-rmi}. Once the user's program has terminated, the server wraps the result in a JSON object and returns it to the front-end.

\img{images/42-editor-template.png}{One of the challenges in the bug bounty programme}

The above screenshot shows one of the challenges that users can attempt to solve as part of the bug bounty programme. The code on the left-hand side is modifiable, while the code on the right-hand side reflects the complete program (note that the 'Code' section contains the copied user input). In this challenge, users need to try to modify the 'Square' instance, which would break the immutability guarantee of 42. At the bottom we can see the program output, which currently displays an exception that was thrown because the challenge had not been solved.

The next screenshot shows all currently available sample programs and challenges. The list is subdivided into three sections: sample programs, intended to guide users to understand how 42 programs work in general; tutorial challenges, which are designed to be easily solvable by learners of 42; and finally Bug Bounty Challenges, which are meant to be unsolvable except via a bug in the 42 implementation. The bug bounty challenges are described in more detail in the appendix.

\img{images/42-editor-sample-programs.png}{42 sample programs and challenges}

\section{Adding Additional Challenges}

Additional challenges can easily be added by simply modifying the front-end code. It includes a JavaScript/JSON configuration file \code{editor/examples.js} that contains all built-in programs and challenges. New bug bounty challenges can be inserted into this configuration file, without requiring updates to the server-side (back-end) code.

\img{images/examples-js.png}{Configuration of bug bounty challenges}

\chapter{Design, Implementation and Deployment}
...
\chapter{High Availability and Security}

This chapter describes how to ensure our solution is production-ready. The bug bounty server will be publicly available on the internet, with a potentially high number of users. Therefore the server needs to be able to handle large numbers of requests, as well as be protected from malicious attacks. We also need to consider that legitimate users, who may accidently submit problematic code, such as infinite loops.

We will be considering malicious code injection, how to respond to overloading the server with requests (DoS), implementing timeouts to prevent infinite loops, and how to increase server performance by leveraging caching.

\section{Preventing Malicious Code Injection}\label{C:injection}
...
\section{Handling Parallel Requests}
...
While the REPL already works on mobile devices, including phones and tablets, it might be worth further optimising the user interface for small screens. We could also work more on accessibility features. Code autocompletion would also be a useful feature to add in the future.
...
These are the bug bounty challenges. In each case, users are only allowed to modify the \code{Code} block. The goal of each of these challenges is to cause the \code{"--secret--"} string to be printed, which is intended to only be possible if users are able to exploit bugs in the 42 implementation.
...
\begin{lstlisting}[caption=Bug Bounty Challenge \#3]
reuse [L42.is/AdamsTowel]
IsOK = {interface read method Bool isOk()}
A = Data:{
