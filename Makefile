...
build: clean
	...
	docker build --network=host --tag l42-docker-controller docker

run:
	docker run --publish 8000:80 -it l42-docker-controller

deploy:
	./deploy.sh

docker_debug:
	docker run --entrypoint /bin/sh -it l42-docker-controller
